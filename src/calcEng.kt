...
fun main() {
    ...
    while (!toStop) { // 반복문
        val className = getThing("Subject") // 과목명 입력
        val learners = getThing("The number of Examinees").toInt() // 수강자 수 입력
        var placement = getThing("Rank").toDouble() // 석차 입력
        val samePlaces = getThing("The number of Same Rank").toInt() // 동석차 수 입력
        when (samePlaces != 0) {
        ...
        print("Input Y to Stop : ")
        when (readLine()!! == "Y") {
            true -> toStop = true // "Y"가 입력되면 toStop 변수를 true로 바꾸어 반복을 멈춘다.
        }
    } // 반복문 종료
    for (i in list.indices) { // 리스트 전체
        sb.append("${list[i].first} : ${list[i].second}th Grade") // "{과목명} : {등급 변수}등급" 스트링 빌더에 추가
        sb.append("\n") // 줄바꿈
    }
    print("\n") // 줄바꿈
...
private fun getThing(input : String) : String { // 입력 함수, 파라미터 input을 넣어 출력 후 입력받은 값을 리턴한다.
    print("Please Input ${input} : ") // 파라미터 input을 넣어 출력
